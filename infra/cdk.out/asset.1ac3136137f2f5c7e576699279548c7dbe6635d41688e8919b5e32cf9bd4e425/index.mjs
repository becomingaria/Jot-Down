...
export async function handler(event) {
  try {
    ...
    if (method === 'GET' && path === '/wikis/{wikiId}/files/{fileId}/export') {
      ...
      if (format === 'docx') {
        ...
        const docxContent = generateSimpleDocx(content, metaResult.Item.name);
    ...
    if (method === 'GET' && path === '/wikis/{wikiId}/folders/{folderId}/export') {
...
function generateSimpleDocx(markdownContent, title) {
  ...
  return Buffer.from(`Markdown export: ${title}\n\n${markdownContent}`, 'utf-8');
}

async function exportFolderAsZip(wikiId, folderId) {
  // Get folder info
  const folderResult = await ddb.send(new GetCommand({
    TableName: TABLE_NAME,
...
async function exportWikiAsZip(wikiId) {
  ...
  const folders = (allItems.Items || []).filter(i => i.entityType === 'folder');
  const fileMetas = (allItems.Items || []).filter(i => i.entityType === 'file');

 ...
 */
export function useCollaboration({ wikiId, fileId, accessToken }) {
  ...
  const startPing = useCallback((ws) => {
    ...
    }, 8 * 60 * 1000) // 8 min — keeps alive under API GW 10 min idle timeout
  }, [])
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (msg.type === "file.update") {
          // Only surface updates for the currently open file
          if (
  ...
  return { connectionStatus, remoteUpdate, clearRemoteUpdate }
}
